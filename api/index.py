...
def get_ai_analysis(resume_text: str, jd_text: str) -> dict:
    if not llm:
        return {"score": 0, "missing_skills": ["LLM not available."]}
    
    failure_response = {"score": 0, "missing_skills": ["AI analysis failed."]}
    
    try:
        # This new prompt asks the AI to intelligently extract skills and find what's missing.
        prompt = PromptTemplate.from_template(
            """Act as an expert hiring manager. Your task is to analyze the resume against the job description.
Your response MUST be a single, valid JSON object and nothing else.
The JSON object must contain two keys: "score" and "missing_skills".

- "score": An integer (0-100) representing the overall match quality.
- "missing_skills": An array of strings listing the key skills, technologies, or qualifications that are mentioned in the job description but are MISSING from the resume.
...
"""
        ...
        if not json_match:
            print(f"[api] AI response malformed: {response_text}")
        ...
        print(f"[api] Critical error during AI call: {e}")
        return failure_response
    ...
    def do_POST(self):
        try:
            ...
            if isinstance(job_description_data, str) and job_description_data.startswith("data:"):
                ...
                if 'pdf' in header.lower(): job_description_text = extract_text_from_pdf(jd_bytes)
                elif 'wordprocessingml' in header.lower(): job_description_text = extract_text_from_docx(jd_bytes)
            else:
            ...
            if not resume_text: raise ValueError("Could not read resume text.")
            ...
            total_score = ai_results.get("score", 0)
            ...
            result = {
                ...
                "score": total_score,
                "verdict": verdict,
                "missingSkills": ai_results.get("missing_skills", [])
            }
